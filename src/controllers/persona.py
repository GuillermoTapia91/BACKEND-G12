...
class PersonasController(Resource):
    def get(self):
        ...
        for persona in data:
          if persona.get('foto'):  
            persona['foto']=S3.generate_presigned_url('get_object',Params={'Bucket':environ.get('AWS_BUCKET_NAME'),'Key':persona.get("foto")},ExpiresIn=50)
    ...
    def post(self):
        ...
        try:
          ...
          if foto:
              ...
              foto.save(ruta)
              respuesta =S3.upload_file(ruta,environ.get('AWS_BUCKET_NAME'), nombreFoto)
              print(respuesta)
          ...
          if portada:
              ...
              portada.save(ruta) 
              S3.upload_file(ruta,environ.get('AWS_BUCKET_NAME'), nombrePortada)
          
          
          nuevaPersona =Persona(nombre=dataValidada.get('nombre'), foto=nombreFoto,portada=nombrePortada)  
